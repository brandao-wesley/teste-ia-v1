...
Branch base: main
Branch de trabalho: feature-dev-agent-api-de-clientes-teste-dev-agent-task-18
Branch de passagem: não usada
Commit original/estado inicial: 27dcc4236575db89c8f5864514f04a954a22887a
Branch backup local: backup/dev-agent-task-18-before
Tag rollback local: rollback/dev-agent-task-18-before
...
1. Fechar o PR.
2. Excluir a branch `feature-dev-agent-api-de-clientes-teste-dev-agent-task-18` se desejar.
...
git checkout main
git pull
git checkout -b rollback/dev-agent-task-18
git revert <merge_commit>
git push origin rollback/dev-agent-task-18
```

## Se precisar voltar ao estado inicial local
```bash
git checkout main
git reset --hard rollback/dev-agent-task-18-before
```
